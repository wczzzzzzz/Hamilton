...
prescribe!(elements["Ω"],:α=>(x,y,z)->β)
...
prescribe!(elements["Ω∩Γ₃"],:α=>(x,y,z)->β)
prescribe!(elements["Ω∩Γ₄"],:EA=>(x,y,z)->EA)
prescribe!(elements["Ω∩Γ₄"],:ρA=>(x,y,z)->ρA)
...
# 𝑎ˢ = stabilization_bar_LSG=>elements["Ω"]
# 𝑎ˢ = stabilization_bar_LSG=>elements["Ω∩Γ₃"]
# 𝑎ˢ = stabilization_bar_LSG=>elements["Ω"]∪elements["Ω∩Γ₃"]
...
# dt = [k+kᵅ+kˢ -k;-k kᵝ]\[fᵅ;-f+fᵝ]
# dt = [k+kᵅ+β.*kᵖ -k-β.*kᵖ;-k kᵝ]\[fᵅ;-f+fᵝ]
# dt = [k+kᵅ+kˢ+β.*kᵖ -k-β.*kᵖ;-k kᵝ]\[fᵅ;-f+fᵝ]
d = dt[1:nₚ]
δd = dt[nₚ+1:end]
