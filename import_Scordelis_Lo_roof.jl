...
function import_roof_Tri3(filename::String)
    gmsh.initialize()
    gmsh.open(filename)
    ...
    elements["Γᵍ"] = getElements(nodes, entities["Γᵍ"], Element{:Seg2}, integrationorder)
    elements["Γᵗ"] = getElements(nodes, entities["Γᵗ"], Element{:Seg2}, integrationorder)
