...
prescribe!(elements["Ωᵍ"],:∂u∂x=>(x,y,z)->∂𝑢∂x(x,y))
prescribe!(elements["Ωᵍ"],:∂u∂y=>(x,y,z)->∂𝑢∂t(x,y))
...
# 𝑎ᵖ = ∫q∇𝑛pds=>elements["Ω∩Γ₃"]∪elements["Ω∩Γ₄"]
...
# dt = [k+β.*kᵖ+kᵅ -k-β.*kᵖ;-k kᵝ]\[fᵅ;-f+fᵝ]
...
cells = [MeshCell(VTKCellTypes.VTK_TRIANGLE,[x.𝐼 for x in elm.𝓒]) for elm in elements["Ω"]]
vtk_grid("./vtk/bar_"*string(ndiv)*".vtu",points,cells) do vtk
    vtk["d"] = [node.d for node in nodes]
end

println(log10(4/ndiv))
println(𝐿₂)
println(𝐻₁)